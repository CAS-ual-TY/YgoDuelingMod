...
public class PlayFieldTypes
{
    public static final PlayFieldType DEFAULT = new PlayFieldType()
        ...
        .newInteraction().icon(ActionIcons.SPECIAL_SUMMON_SET).interactorIncluded(PlayFieldTypes.getAllStackZones()).interactorCardNotNull().interacteeIncluded(PlayFieldTypes.getAllMonsterZones()).interaction((player, interactor, card, interactee) -> new MoveTopAction(ActionTypes.MOVE_ON_TOP, interactor, card, interactee, CardPosition.SET, player)).playerAndInteractorSameOwner().interacteeEmpty().addInteraction()
